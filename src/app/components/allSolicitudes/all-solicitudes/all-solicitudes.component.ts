...
@Component({
  ...
  styleUrl: './all-solicitudes.component.css',
...
export class AllSolicitudesComponent implements OnInit{
  ...
  setEstadoTrackingReqModel: SetEstadoTrackingReqModel = new SetEstadoTrackingReqModel();
  ...
  buscar(){
    ...
    // call servicio
    this.envioService.getEstadoEnviosSearchAdmin(this.getEstadoEnvioReqModel).subscribe(
      (response) =>{
        this.spinner.show();
  ...
  buscarAll(){
    ...
    // call servicio
    this.envioService.getEstadoEnviosAdmin(this.getEstadoEnvioReqModel).subscribe(
      (response) =>{
        this.spinner.show();
        this.alltable = response
  ...
  setEstadoTracking(model: any,_estado: number){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if(model.estado_pago === "PENDIENTE" && _estado === 4){
          pendientePago = 1;
        }

        this.setEstadoTrackingReqModel = new SetEstadoTrackingReqModel();

        this.setEstadoTrackingReqModel = {
          idEnvio: model.id_envio,
          estado : _estado,
          ...
          telefonoEmpresa: model.telefono_origen,
          nOrden: model.n_orden,
        }
     
        // call servicio
        this.envioService.setTracking(this.setEstadoTrackingReqModel).subscribe(
          (response) =>{
            this.spinner.show();
            this.respuestaGlobalModel = response;
